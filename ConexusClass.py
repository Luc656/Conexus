...
class Conexus:
    ...
    def annotate(self):
        ...
        if self.prokka not in self.current:
            # PROKKA command
            subprocess.call(['prokka', '--outdir', self.prokka, '--prefix', self.prokka, self.file])
    ...
    def statement(self):
        ...
        for i in self.noProkka:
            if not self.noProkka[i]:
                noFAA.append(i)

        print(bordered(f'Of {len(self.fastas)} fasta files there were {len(self.fastas)-len(noFAA)} annotated properly'))
...
class ConexusGroup():
    ...
    def extractCoin(self):
        ...
        with open(f'{prefix}_pairs.tsv', 'r') as f1, open(f'gene_presence_absence-withquotes.csv', 'r') as f2:
