...
class Conexus:
    ...
    def checkProkka(self):
        global noProkka
        self.prokkaCount += 1
        noProkka.update({self.file:''})
        os.chdir(self.file)
        for subFile in os.listdir():
            if subFile.split('.')[-1] == 'faa':
                noProkka[self.file] = True
        os.chdir('..')

    def checkRGI(self):
        global noRGI
        self.rgiCount += 1
        noRGI.update({self.file:''})
        os.chdir(self.file)
        for subFile in os.listdir():
            if subFile.split('.')[-1] == 'txt':
                noRGI[self.file] = True
        os.chdir('..')
...
class ConexusGroup():
    ...
    def extractCoin(self):
        ...
        print(bordered(f'Conexus found {len(hits)} key gene cluster pairing(s) with a link to AMR'))
    ...
    def statement(self):
        ...
        with open('stats.txt', 'w') as stats:
            stats.write(f'Of {len(fastas)} fasta files there were {len(fastas)-len(noFAA)} annotated properly')
            stats.write('\n')
            stats.write(f'Files not properly annotated: {noFAA}')
            stats.write('\n')
            stats.write(f'Of {len(fastas)} fasta files there were {len(fastas) - len(noTXT)} scanned for AMR genes properly')
            stats.write('\n')
            stats.write(f'Files not properly scanned for AMR genes: {noTXT}')
...
if __name__=='__main__':
    ...
    print(bordered(f'New csv with all {len(df)} AMR genes discovered is ready'))

    for file in os.listdir():
        if file.split('.')[-1] == 'prokka':
            individual = Conexus(file)
            individual.checkProkka()
    ...
    group.statement()
    #
    # os.system('mkdir prokka.and.RGI.Files')
    #
    # for i in os.listdir():
    #     if i.split('.')[-1] == 'prokka' or i.split('.')[-1] == 'rgi':
    #         os.system(f'mv {i} prokka.and.RGI.Files')
